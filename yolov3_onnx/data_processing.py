...
# Let's make sure that there are 80 classes, as expected for the COCO dataset:
CATEGORY_NUM = len(ALL_CATEGORIES)
assert CATEGORY_NUM == 80 

class PreProcessYOLO(object):
    """A simple class for loading images with PIL and reshaping them 
    to the specified input resolution for YOLOv3-608."""

    def __init__(self, yolo_input_resolution):
        """Initialize with the input resolution for YOLOv3, which will stay 
        fixed in this sample.
        
        Keyword arguments:
            yolo_input_resolution -- two-dimensional tuple with the target network's 
            (spatial) input resolution in HW order
        """
    ...
    def process(self, input_image_path):
        ...
        and return it together with a pre-processed version
        required for feeding it into a YOLOv3 network.
...
class PostprocessYOLO(object):
    ...
    def __init__(
        ...
        Keyword arguments:
            yolo_masks -- a list of three-dimensional tuples for the YOLO masks
            ...
            input_resultion_yolo -- two dimensional tuple with the target network's (spatial)
                input resulution in HW order
            """
    ...
    def process(self, outputs, resolution_raw):
        """Take the YOLOv3 outputs generated from a TensorRT forward pass, post-process
        them and return a list of bounding boxes for detected object together with their
        category and their confidences in separate lists.
        ...
        for output in outputs:
            outputs_reshaped.append(self.__reshape_output(output))
    ...
    def _reshape_ouput(self, output):
        """Reshape a TensorRT output from NCWH to NHWC format (with expected C=255), 
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        """Take in a list of three reshaped YOLO outputs in (height, width, 3, 85)
        shape and return a list of bounding boxes for detected object together with
        their confidence in separate lists. 
        
        Keyword arguments:
            outputs_reshaped -- list of three reshaped YOLO outputs as Numpy as array 
            with shape (height, width, 3, 85)
        resolution_raw -- the original spatial resolution form the input PIL image in WH order 
        """
        # E.g. in YOLOv3-608, there are three output tensors, which we associate with their
        # respective masks. Then we iterate throught all output-mask pairs and generate candidates
        # for bouding boxes, their corresponding category predictions and their confidences:
    ...
    def _process_feats(self, output_reshaped, mask):
        """Take in a reshaped YOLO output in height,width,3,85 format
        together with its corresponding YOLO mask and return the detected
        bounding boxes, the confidence, and the class probability in each 
        cell/pixel.
        ...
        col = np.title(np.arange(0, grid_w), grid_w).reshape(-1, grid_w)
        row = np.title(np.arange(0, grid_h).reshape(-1, 1), grid_h)
        ...
        # boxes: controids, box_confidence: confidence level, box_class_probs:
        # class confidence 
        return boxes, box_confidence, box_class_probs

    def _filter_boxes(self, boxes, box_confidences, box_class_probs):
        """Take in the unfiltered bounding boxes descriptors and discard each
        cell whose score is lower than the object threshold set during intialization. 
        
        Keyword arguments:
            boxes -- bounding box coordinates with shape (height,width,3,4); 4 for
            x,y,height,width coordinates of the boxes 
            box_confidence -- bounding box confidence with shape (height,width,3,1); 1 
            for as confidence scalar per element
    ...
    def _nms_boxes(self, boxes, box_confidences):
        ...
        boxes with their confidence scores and return an array with the indexes
        of the bounding boxes we want to keep (and display later).
        
        Keyword arguments:
            boxes -- a Numpy array containing N bounding-box coordinates that 
            survived filtering, with shape (N,4); 4 for x,y,height,width coordinates of the boxes
        ...
        while ordered.size > 0:
            ...
            # The goal of the NMS Algorithm is to reduce the number of adjacent 
            # bounding-box candidates to a minimum. In this step, we keep those 
            # elements whose overlap with the bounding box is lower than the threshold:
