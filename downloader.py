...
def download(data_dir, yaml_path, overwrite=False):
    ...
    Return false if the downloaded file or the local copy (if not overwrite) 
    ...
    def _downloadFile(path, url):
        ...
        try:
            ...
            logger.debub("Error occurred while requesting connection to %s: %s.", url, e)
    allGood = True 
    for f in sample_data.files:
        ...
        if os.path.exists(fpath):
            if _checkMD5(fpath, f.checksum):
                ...
                if overwrite:
                    logger.warning("Removing local copy %s", fpath)
                    os.remove(fpath)
                else:
                    allGood = False 
                    continue
...
def _parseArgs():
    ...
    args, _ = parser.parser_know_args()
    data = os.environ.get("TRT_DATA_DIR", None) if args.data is None else args.data 
    if data is None:
        raise ValueError(
            "Data directory must be specified by either `-d $DATA` or enviroment variable $TRT_DATA_DIR."
        )
    return data, args 
...
def verifyChecksum(data_dir, yaml_path):
    ...
    for f in sample_data.files:
        ...
        if os.path.exists(fpath):
            ...
            allGood = False
            logger.error("Data file %s does not have a local copy", f.path)
    return allGood 
...
def getFilePath(path):
    ...
    It only works when the sample doest not have any other command line arguments."""
    global TRT_DATA_DIR
    if not TRT_DATA_DIR:
        parser = argparse.ArgumentParser(
    ...
    if not os.path.exists(fullpath):
        raise ValueError("Data file %s does not exist!" % fullpath)
